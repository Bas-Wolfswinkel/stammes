        ...
        <div class="grid grid-cols-1 gap-12 pt-[35px] uppercase lg:grid-cols-4">
            @if ($logo = get_field('logo', 'option'))
                <a class="mx-auto block w-full md:mx-0" href="{{ home_url('/') }} order-1">
                    <img class="h-full max-h-[92px] w-full max-w-[312px] object-contain" src="{{ $logo['url'] }}" alt="{{ $logo['alt'] }}">
                </a>
                @if (has_nav_menu('header_navigation'))
                    <nav class="order-2 hidden space-y-[6px] lg:block" aria-label="footer">
                    ...
                    <div class="order-4 space-y-[6px] lg:order-3">
                        ...
                        <ul class="list-none space-y-[6px]">
                            @foreach ($services as $service)
                                <li>
                                    <a class="font-lato text-button text-white" href="{{ get_the_permalink($service) }}">{{ $service->post_title }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                    ...
                    <div class="order-3 space-y-[6px] lg:order-4">
                        ...
                        <ul class="list-none space-y-[6px]">
                            ...
                            @if ($contact_arr['email'])
                                <li>
                                    <a class="font-lato text-button text-white" href="mailto:{{ $contact_arr['email'] }}">{{ $contact_arr['email'] }}</a>
                                </li>
                            @endif
                            @if ($contact_arr['phone'])
                                <li>
                                    <a class="font-lato text-button text-white" href="tel:{{ $contact_arr['phone']['url'] }}">{{ $contact_arr['phone']['title'] }}</a>
                                </li>
                            @endif
                        </ul>
                    </div>
